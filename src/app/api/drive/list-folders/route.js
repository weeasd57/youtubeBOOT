...
export async function GET(req) {
  try {
    ...
    try {
      ...
      console.log(`Found ${folders.length} folders, request took ${Date.now() - startTime}ms`);
      
      // Log all folders for debugging
      if (folders.length > 0) {
        console.log('Folders found in Drive:');
      ...
      if (error.code === 401 || 
        ...
        return NextResponse.json(
          { error: 'Authentication error. Please refresh your Google authentication.' },
          { status: 401 }
        );
      }
    ...
    console.error('Unhandled error in Drive list-folders endpoint:', error);
    return NextResponse.json(
